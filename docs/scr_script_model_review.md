...
This note rechecks the structural/VM model strictly against the decompiled sources in `src/` and marks what’s confirmed, what needs correction, and what remains hypothesis.
...
- Current working alias model: `pbGpffffbd60` and `DAT_00355cd0` appear to be the same GP-relative script PC rendered under two decompiler names; `uGpffffbd68` and `DAT_00355cd8` appear to be the same current-opcode word.

## Corrections/nuances

- Order at `BLOCK_OPEN (0x32)`: The structural interpreter calls `FUN_0025c220` immediately after setting `pb` to `(start + 1)` and pushing continuation `(start + 5)`. With the PC alias above, the four bytes after `0x32` are read as a little-endian signed self-relative delta by `FUN_0025c220`.
- Effective offline rule: `target = opcode_offset + 1 + s32(opcode_offset + 1)`, with continuation `opcode_offset + 5` saved for later `0x04` unwinds.
...
- Wire-format marker: pattern `0x0B 0x04 <ID16>` appears in files. While not named in code, we can surface these as plausible SUBPROC tags in scans for correlation.
- Disassembler policy: do not annotate `0x32` rel32 cells as SUBPROC IDs. Scan `0x0B 0x04 <ID16>` separately and keep it clearly labeled as derived from data, not asserted semantics.
...
- Structural layer:
  - Start at a known block offset. On `0x32`, read `s32(pc + 1)`, push continuation `(pc + 5)`, and continue at `pc + 1 + rel32`. On `0x04`, pop continuation or, if at top-most frame, stop.
...
In short: the high-level model is largely sound; correct the `0x32` ordering and treat the following four bytes as the self-relative delta consumed by `FUN_0025c220`. Treat SUBPROC as a real engine/debug concept; scan likely on-disk subproc tags (`0x0B 0x04 <ID16>`) while keeping their exact semantics provisional until tied to handlers.
