...
0x4C set_camera_roll  # orig FUN_0025e520 — eval expr, normalize by DAT_00352b8c (100000.0), store in DAT_0035564c. That global is gp - 0x492C = `uGpffffb6dc`, the camera roll FUN_0020bec8 feeds the view matrix — not a projection distance. Same global as 0xBE's storm roll (FUN_002676d8) and a camera path's roll/zoom curve (FUN_00217fe8); scripts use it to put a banked camera back level. See analyzed/ops/0x49_0x4A_0x4B_0x4C_projection_system.c
...
0x8B conditional_map_change  # orig FUN_00260e30 — read 7 exprs (mask, major, minor, flags, XYZ); gate on mask vs lead flags DAT_0058bf1c and on grounded unless flags&0x10000; normalize coords by fGpffff8cb4, call FUN_0022b2c0; returns 1 if the warp was requested; see analyzed/ops/0x8B_conditional_map_change.c. NOT audio — corrected, FUN_0022b2c0 is the map-change request
0x8C request_map_change  # orig FUN_00260f78 — read 6 exprs (major, minor, flags, XYZ), no gating beyond "no change already pending"; normalize coords by fGpffff8cb8, call FUN_0022b2c0; flags 0x200000/0x02 side effects; returns 0; see analyzed/ops/0x8C_request_map_change.c. NOT audio — corrected
...
0x94 start_camera_shake  # orig FUN_002612e0 — magnitude = expr0/DAT_00352c34, duration = (short)expr1, into FUN_0022dcf0 -> fGpffffb6f4/uGpffffb6f8. Not audio: FUN_0020bec8:0x0020bfc4 is the only consumer and adds min(remaining*0.0003125, magnitude)*sinf(remaining/40) to the camera eye height. The old "audio positioning" name came from FUN_0022dcf0's tail call to FUN_0023baf8, which retail leaves empty; see analyzed/ops/0x94_start_camera_shake.c
...
0xB1 bind_entity_action  # orig FUN_00263878 — actually clear_bone_override: FUN_0020d9c8 zeroes +0x168[bone], then +0x08 |= 0x10 (next pose sample snaps); see [analyzed/ops/0xB1_bind_entity_action.c](../analyzed/ops/0xB1_bind_entity_action.c)
0xB2 set_entity_anim_rate  # orig FUN_002638d8 — actually set_bone_scale: samples bone pose (FUN_0020da68), field 6 (scale) = v/100000, installs via FUN_0020d8c0 duration 0; see [analyzed/ops/0xB2_set_entity_anim_rate.c](../analyzed/ops/0xB2_set_entity_anim_rate.c)
0xB3 set_entity_anim_xyz_rate  # orig FUN_00263978 — actually set_bone_translation: fields 3..5 (translation xyz) = v/100000, same sample/override path as 0xB2; see [analyzed/ops/0xB3_set_entity_anim_xyz_rate.c](../analyzed/ops/0xB3_set_entity_anim_xyz_rate.c)
0xB4 set_entity_anim_channel  # orig FUN_00263a58 — actually set_bone_pose_field: selector, bone, inline field byte, value; field 0..2 rotation, 3..5 translation, 6 scale, v/100000; see [analyzed/ops/0xB4_set_entity_anim_channel.c](../analyzed/ops/0xB4_set_entity_anim_channel.c)
...
0xB8 set_draw_distance  # orig FUN_00263cb8 — eval expr, normalize by fGpffff8d44 (100000.0), set DAT_0032538c and fGpffffb6b8 = DAT_00355628, the DRAW distance the visibility pass culls against, plus fGpffffb6bc = DAT_0035562c (distance-5.0). Not a camera distance: FUN_0022a418:185 assigns DAT_00355628 = DAT_0032538c and :354 derives DAT_0035562c the same way. Part of the scene environment block; see analyzed/ops/0xB8_set_camera_distance.c
0xB9 set_fog_colour  # orig FUN_00263d10 — packs RGB into uGpffffb704 = DAT_00355674, the fog colour (0..255 per channel). FUN_0022a418:346 defaults it to 0x505050. Scene environment block; see [analyzed/ops/0xB9_set_global_color1_rgb.c](../analyzed/ops/0xB9_set_global_color1_rgb.c)
0xBA set_fog_colour2  # orig FUN_00263d60 — packs RGB into uGpffffb708 = DAT_00355678, set alongside 0x355674 and to the same value by both FUN_0022a418 (0x505050) and FUN_0026f108 (0xfed261). Its distinct consumer is not identified; see [analyzed/ops/0xBA_set_global_color2_rgb.c](../analyzed/ops/0xBA_set_global_color2_rgb.c)
0xBB set_fog_band  # orig FUN_00263db0 — fGpffffb70c = DAT_0035567c (fog near) and fGpffffb710 = DAT_00355680 (fog far), both / fGpffff8d48 (100000.0). FUN_0022a418:344,347 default them to drawDistance*0.25 and drawDistance. Warns when near < 2.0. Scene environment block; see [analyzed/ops/0xBB_set_fade_radius_pair.c](../analyzed/ops/0xBB_set_fade_radius_pair.c)
0xBC give_item  # orig FUN_00263e30 — see [analyzed/ops/0xBC_give_item.c](../analyzed/ops/0xBC_give_item.c)
...
0xC8 set_screen_smear_alpha  # orig FUN_00264448 — low byte of one expression into DAT_00355661 (`bGpffffb6f1`). **Corrected:** not a text colour index. Nothing in the dialogue or menu renderer reads DAT_00355661; its readers are FUN_002000c0:214, which gates on it, and FUN_00201a38, the full-screen frame-feedback blit that draws the *previous* framebuffer back over this one. Alpha is a GS blend factor over 128, so the 0x7E the ship scenes use is 98% of the previous frame, compounding into a smear. See analyzed/ops/0xC8_set_screen_smear_alpha.c and analyzed/frame_feedback_blit.c
0xC9 set_screen_smear_alpha_and_transform  # orig FUN_00264470 — 0xC8 plus five shorts into DAT_00343878: offsetX, offsetY, scaleX, scaleY, rotation (tenths of a degree). **Corrected:** not a palette, and it reads six expressions, not seven — Ghidra splits the sixth into a second local that is really the same array + 0x10. FUN_00201a38 transforms only the destination quad with these, never the source rect, so scale gives a zoom blur, rotation a swirl and offset a directional smear. See analyzed/ops/0xC9_set_screen_smear_alpha_and_transform.c
...
0xE6 set_backdrop_slot_shade  # orig FUN_00265200 — the four-entry table is background models, not minimap markers; see [analyzed/ops/0xE6_set_backdrop_slot_shade.c](../analyzed/ops/0xE6_set_backdrop_slot_shade.c)
...
0x10E open_plume_emitter  # orig FUN_00262a98 — opens one plume emitter (the tenth particle pool: fire + smoke); five of ten operands scaled by fGpffff8d10 → FUN_0021f6e8; see [analyzed/ops/0x10E_open_plume_emitter.c](../analyzed/ops/0x10E_open_plume_emitter.c) and [analyzed/plume_particle_pool.c](../analyzed/plume_particle_pool.c)
